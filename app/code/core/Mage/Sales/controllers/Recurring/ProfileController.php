        ...
        try {
            ...
            switch ($this->getRequest()->getParam('action')) {
                ...
                case 'activate':
                    ...
                    break;	
					//6-8-2016 by Chris.
				case 'schedule_to_freeze':
					$profile->scheduleToFreeze($profile->getId(),$_GET["s"], $_GET["e"]);
					break;
				case 'schedule_to_freeze_cancel':
					$profile->scheduleToFreezeCancel($profile->getId(),$_GET["s"], $_GET["e"]);
					break;
            }
			if($redirect){
            ...
            $this->_session->addError($this->__('Failed to update the profile.') . $e ."xxx".$_GET["s"], $_GET["e"]);
            Mage::logException($e);
        }
        if ($profile) {
