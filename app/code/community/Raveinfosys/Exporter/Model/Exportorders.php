...
class Raveinfosys_Exporter_Model_Exportorders extends Raveinfosys_Exporter_Model_Exporter
{
    ...
    protected function getHeadRowValues()
    {
		...
		$headerForAttributes = array();
		foreach($attributesCollection as $attribute){
			array_push($headerForAttributes, $attribute->getFrontendLabel());
		}
		//print_r($attribute->getFrontendLabel());
		return array_merge($header,$headerForAttributes);
    }
    ...
    protected function getOrderItemValues($item, $order, $itemInc = 1)
    {
		...
		$attributesCollection = Mage::getResourceModel('catalog/product_attribute_collection');
		$attributesCollection->setAttributeGroupFilter(18);  //ingredientsGroup
		$itemAttributes = $this->getItemsIngredientAttributes($item, $attributesCollection);
		//var_dump($attributesCollection);
		//die;
		
		return array_merge($items, $itemAttributes);
    }
	
	//5-31-2016 by Chris.  Load CURRENT Product Attribute on EXPORTER EXTENSION
	protected function getItemsIngredientAttributes($item, $attributesCollection){
