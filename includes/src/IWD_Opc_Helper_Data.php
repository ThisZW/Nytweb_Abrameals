	...
	public function getSubscriptionTotal(){
		...
		foreach ($items as $item) { 
			if($item->getProduct()->getIsRecurring()){
				...
				$period = $item->getProduct()->getRecurringProfile('period_unit');
				Mage::log($price . ' ' . $period, null, 'test2.log', true);
				$str = 'Plus ' . $price . '/' . $period .'.';
			return $str;
			//Mage::log($item->getProduct()->debug(), Zend_log::DEBUG,  'test.log', true);
		}
	}	
